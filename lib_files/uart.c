...
void UART_SetBaudRate(uint32_t var_baudRate)
{
	...
	{
		regVal = M_GetBaudRateValue(var_baudRate);
	}
	
	else
	{
		regVal = M_GetBaudRateValue(9600);
	}
	
	UBRRL = ExtractByte0To7(regVal);
	UBRRH = ExtractByte8To15(regVal);
}
